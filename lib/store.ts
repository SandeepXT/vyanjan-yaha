...
export function subscribeToOrder(orderId: string, listener: SSEListener): () => void {
  ...
  return () => {
    sseListeners.get(orderId)?.delete(listener);
  };
}
...
export function validatePlaceOrderPayload(payload: unknown): ValidationError[] {
    ...
    (p.items as unknown[]).forEach((item, index) => {
      ...
      if (!i.menuItemId || typeof i.menuItemId !== "string") {
        ...
        errors.push({
          ...
          message: `Menu item '${i.menuItemId}' not found`,
        });
      }
  ...
  if (!dd || typeof dd !== "object") {
    ...
    if (!dd.name || typeof dd.name !== "string" || dd.name.trim().length < 2) {
      errors.push({
    ...
    if (!dd.phone || typeof dd.phone !== "string" || !phoneRegex.test(dd.phone.replace(/\s/g, ""))) {
      errors.push({
    ...
    if (!dd.address || typeof dd.address !== "string" || dd.address.trim().length < 10) {
      errors.push({
...
/* ─────────────────────────────────────────────────────────────────────────────
   Auto Status Progression (FIXED FOR JEST)
───────────────────────────────────────────────────────────────────────────── */
...
function scheduleStatusProgression(orderId: string) {
  STATUS_SEQUENCE.forEach((status) => {
    ...
    // ✅ Prevent Jest from waiting for long timers
